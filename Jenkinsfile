...
node("docker") {
	stage("Setup Build Environment") {
		checkout scm
		sh "docker pull registry.gentics.com/jenkins-slave-mesh-openjdk11"
		sh "cd .jenkins && docker build -t registry.gentics.com/jenkins-slave-mesh-openjdk11 ."
		sh "cd .jenkins && docker push registry.gentics.com/jenkins-slave-mesh-openjdk11"

		podTemplate(containers: [
			containerTemplate(alwaysPullImage: true, args: '${computer.jnlpmac} ${computer.name}',
				command: '',
				image: 'registry.gentics.com/jenkins-slave-mesh-openjdk11',
				name: 'jnlp',
				privileged: false,
				ttyEnabled: true,
			...
			containerTemplate(alwaysPullImage: false, 
				...
				instanceCap: 10,
				label: 'mesh',
				name: 'jenkins-slave-mesh-openjdk11',
				namespace: 'default', 
